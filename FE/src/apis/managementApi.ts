...
export const RDDEFAULTTIME = (userId: number) => `${DEFAULTTIME}?userId=${userId}`; //기본 상담시간 조회, 삭제
...
export const DELETEUNAVAILABLETIME = (idx: number) => `${UNAVAILABLE}?unavailableTimeIdx=${idx}`; //상담 불가능 시간 제거
export const READUNAVAILABLETIME = (counselorId: number) => `${UNAVAILABLE}/${counselorId}`; //상담 불가능 시간 조회
